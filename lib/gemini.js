 ...
 *
 * History is kept in a simple provider-agnostic shape — [{ role: 'user' |
 * 'model', text }] — not Gemini's raw `contents` format. This is what lets
 * lib/groq.js act as a real fallback: both providers read/write the same
 * plain-text history, so a mid-conversation failover doesn't lose context.
 * Tool-call internals (function calls/responses) only ever live inside a
 * single turn's own loop below — they're never persisted across turns.
 *
...
async function runTurn(history, userText, toolSets) {
  ...
  // Build this turn's Gemini `contents` from the plain-text history.
